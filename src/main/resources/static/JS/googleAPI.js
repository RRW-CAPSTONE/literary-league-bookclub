...
(function(){
    $(document).ready(function() {
        // Handling empty search input field form submission
        $('#search-form').on('submit', function(event) {
            event.preventDefault();
            const book = $('#search-input').val();
            searchBooks(book);
        });
        ...
        function searchBooks(book){
            ...
            $.ajax({
                ...
                // Displaying the data in the console
                console.log(data);
                // Calling the displayResults function to create the book cards
                displayResults(data);
                // Clear previous search results
                $('#search-results').empty();
            });
        ...
        function displayResults(data) {
            ...
            $.each(data.items, function(index, item) {
                const title = item.volumeInfo.title;
                const authors = item.volumeInfo.authors ? item.volumeInfo.authors.join(', ') : 'Unknown';
                const thumbnail = item.volumeInfo.imageLinks ? item.volumeInfo.imageLinks.thumbnail : '<img src="https://via.placeholder.com/150" alt="">';
                let description = item.volumeInfo.description ? item.volumeInfo.description : 'No description available.';
                if (description.length > 140) {
                    description = description.substring(0, 140) + '...';
                }
        ...
        $('#book-modal').on('show.bs.modal', function(event) {
            ...
            //const modalDescription = button.data('description');
            ...
            $(this).find('#book-description').val(description);
